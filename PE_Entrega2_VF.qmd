...
pacman::p_load(tidyverse, glue, readxl, janitor, lubridate, scales, data.table, kableExtra, tseries, gt, gtExtras,vars,zoo )
...
# Supongamos que df_diff ya tiene las primeras diferencias y 'llave' es as.yearmon
# Y que predicciones ya fue generado con predict(modelo_var)
...
# Obtener último valor real por variable
ultimo_real <- df_diff %>% 
...
# Crear predicciones dataframe
pred_df <- data.frame(fechas_futuras)
for (var in names(predicciones$fcst)) {
  pred_df[[var]] <- predicciones$fcst[[var]][, "fcst"]
}
pred_df <- pred_df %>%
  rename(llave = fechas_futuras)

# Unir el último punto real al inicio de las predicciones
...
df_real_long <- df_diff %>%
  filter(llave >= (max(df_diff$llave) - (h + 2) / 12)) %>%
