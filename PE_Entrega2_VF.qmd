...
pacman::p_load(tidyverse, glue, readxl, janitor, lubridate, scales, data.table, kableExtra, tseries, gt, gtExtras, zoo, tibble, knitr, gridExtra, corrplot, caret, rpart, rpart.plot, randomForest, xgboost, rlang, prophet, Metrics, forecast,ggfortify, Metrics,patchwork)
```
...
Para el análisis de nuestras variables, hicimos los siguientes modelos:
...
1.  **SARIMA/SARIMAX**
    ...
    -   SARIMAX: Incorpora variables externas que influyen en el comportamiento de las variables dependientes.
...
3.  **Prophet**
    ...
    -   Objetivo: Modelar series de tiempo univariantes capturando automáticamente tendencias no lineales, estacionalidades (diaria, semanal, anual) y efectos puntuales (festividades, promociones).

    -   Ventajas: Es más fácil e intuitivo de utilizar, puede incluir días feriados para ajustar comportamientos atípicos, fácil de interpretar.

    -   Consideraciones: La detección automática de “changepoints” puede sobreadaptarse si hay demasiados cambios bruscos.

4.  **VAR (Vector AutoRegresivo)**

    -   Objetivo: Modelar de forma conjunta varias series temporales para capturar sus interdependencias dinámicas y choques mutuos.

    -   Ventajas: Permite analizar cómo un choque en una variable (p. ej. tipos de interés) impacta a las demás; no exige especificar exógenos.

    -   Consideraciones: Todas las series deben ser estacionarias para evitar resultados engañosos; con muchas variables, el número de parámetros crece rápido (curse of dimensionality); es más complicado de hacer en general (o por lo menos nos costó más trabajo). 

## SARIMA(X)

Usamos modelos SARIMA y SARIMAX para pronosticar la pérdida esperada de portafolio de crédito, como actividad empresarial, nómina y tarjeta. Estos modelos permiten capturar patrones en el tiempo, incluyendo tendencias y cierta estacionalidad. Ajustamos automáticamente, con el uso de auto.arima, el modelo más adecuado para cada serie y generamos predicciones a corto plazo. Aunque el ajuste en el periodo histórico fue aceptable, las proyecciones muestran limitaciones y no logran reflejar por completo los ciclos estacionales observados en los datos.

Para el pronóstico de la **pérdida esperada del portafolio de crédito** se utiliza la forma funcional general de un modelo SARIMA $\ (p,d,q)(P,D,Q)_s$ y se expresa como:
...
\- $\ (theta_q(L)$ y $\ (\Theta_Q(L^s)$ son los polinomios de promedio móvil,
...
```{r}
# Leer datos
arI <- readxl::read_excel("master_base_filtrado.xlsx") %>%
  janitor::clean_names() %>%
  dplyr::mutate(llave = zoo::as.yearmon(as.character(llave), "%Y-%m")) %>%
  dplyr::arrange(llave)

# Variables dependientes
dep_vars <- names(arI)[startsWith(names(arI), "y_")]

# Inicializar
df_all <- list()
intervalos_all <- list()
errores <- tibble::tibble()

# Loop
for (var in dep_vars) {
  try({
    ts_data <- stats::ts(arI[[var]], frequency = 12,
                         start = c(lubridate::year(min(arI$llave)), lubridate::month(min(arI$llave))))
    
    cutoff <- length(ts_data) - 5
    train_ts <- stats::window(ts_data, end = time(ts_data)[cutoff])
    test_ts <- stats::window(ts_data, start = time(ts_data)[cutoff + 1])
    
    fechas_all <- seq(as.Date(zoo::as.yearmon(min(arI$llave))), by = "month", length.out = length(ts_data))
    fechas_train <- fechas_all[1:cutoff]
    fechas_test <- fechas_all[(cutoff + 1):length(ts_data)]
    fechas_forecast <- seq(fechas_test[1], by = "month", length.out = length(test_ts))
    
    # Modelo
    modelo <- forecast::auto.arima(train_ts, seasonal = TRUE, allowdrift = TRUE,
                                   stepwise = FALSE, approximation = FALSE)
    
    fitted_vals <- stats::fitted(modelo)
    forecast_vals <- forecast::forecast(modelo, h = length(test_ts))
    
    df_real <- tibble::tibble(fecha = fechas_train, valor = as.numeric(train_ts), tipo = "Real", variable = var)
    df_fitted <- tibble::tibble(fecha = fechas_train, valor = as.numeric(fitted_vals), tipo = "Ajustado", variable = var)
    df_forecast <- tibble::tibble(fecha = fechas_forecast, valor = as.numeric(forecast_vals$mean), tipo = "Pronóstico", variable = var)
    
    df_int <- tibble::tibble(
      fecha = rep(fechas_forecast, 2),
      valor = c(forecast_vals$lower[, 2], forecast_vals$upper[, 2]),
      bound = rep(c("LI_95", "LS_95"), each = length(forecast_vals$mean)),
      variable = var
    ) %>%
      tidyr::pivot_wider(names_from = bound, values_from = valor)
    
    df_all[[var]] <- dplyr::bind_rows(df_real, df_fitted, df_forecast)
    intervalos_all[[var]] <- df_int
    
    errores <- dplyr::bind_rows(errores, tibble::tibble(
      variable = var,
      rmse = Metrics::rmse(test_ts, forecast_vals$mean),
      mape = Metrics::mape(test_ts, forecast_vals$mean) * 100
    ))
    
    cat(glue::glue("Modelo para {var}: {modelo}\n"))
    
  }, silent = TRUE)
}

# Combinar
df_plot <- dplyr::bind_rows(df_all)
df_ints <- dplyr::bind_rows(intervalos_all)
...
#| fig.asp: 0.8
#| fig.width: 7
ggplot2::ggplot(df_plot, aes(x = fecha, y = valor, color = tipo)) +
  geom_ribbon(data = df_ints, aes(x = fecha, ymin = LI_95, ymax = LS_95), 
              inherit.aes = FALSE, fill = "red", alpha = 0.2) +
  geom_line(size = 0.9) +
  facet_wrap(~ variable, scales = "free_y") +
  scale_color_manual(values = c("Real" = "black", "Ajustado" = "blue", "Pronóstico" = "red")) +
...
Imx <- read_excel("master_base_filtrado.xlsx") %>%
  janitor::clean_names() %>%
  mutate(
    m1              = m1 / 1e6,
    m2              = m2 / 1e6,
    ...
    balanza         = balanza / 1e6,
    remesas         = remesas / 1e3,
    puestos_trabajo = puestos_trabajo / 1e6,
    tiie_fondeo     = tiie_fondeo / 100,
    llave           = zoo::as.yearmon(llave)
  ) %>%
  arrange(llave)

# Definir series objetivo y exógenas
targets <- c(
  "y_pe_actividad_empresarial_o_comercial",
  "y_pe_automotriz",
  "y_pe_entidades_financieras",
  "y_pe_nomina",
  "y_pe_personales",
  "y_pe_tarjeta_de_credito"
)
exogs <- c(
  "usd_mxn","tiie_fondeo","remesas",
  "m1","m2","m3","balanza",
  "inpc","igoec","igae",
  "google_deuda","google_empleo",
  "puestos_trabajo","s_p_bmv_ipc","s_p_500"
)

# 3) Inicializar lista de resultados
df_list_sarimax <- list()

# 4) Loop de modelado SARIMAX
for (tgt in targets) {
  name_pretty <- str_to_title(str_replace_all(tgt, "y_pe_|_", " "))

  df1 <- Imx %>%
    dplyr::select(llave, all_of(tgt), all_of(exogs)) %>%
    drop_na()
  if (nrow(df1) < 24) next

  df1[exogs] <- lapply(df1[exogs], as.numeric)

  ts1   <- ts(df1[[tgt]], start = c(lubridate::year(min(df1$llave)), lubridate::month(min(df1$llave))), frequency = 12)
  xreg1 <- as.matrix(df1[exogs])

  fit_sarimax <- tryCatch(
    forecast::auto.arima(ts1, xreg = xreg1, seasonal = TRUE),
    error = function(e) NULL
  )
  if (is.null(fit_sarimax)) next

  fc_sarimax <- tryCatch(
    forecast::forecast(fit_sarimax, xreg = tail(xreg1, 12), h = 12),
    error = function(e) NULL
  )
  if (is.null(fc_sarimax) || is.null(fc_sarimax$lower) || ncol(fc_sarimax$lower) < 2) next

  df_hist_x <- tibble(
    fecha    = zoo::as.Date(df1$llave),
    valor    = df1[[tgt]],
    variable = name_pretty,
    type     = "Histórico"
  )

  fdates_x <- seq(
    from = max(zoo::as.Date(df1$llave)) %m+% months(1),
    by   = "month",
    length.out = 12
  )
  ...
  df_fc_x <- tibble(
    fecha    = fdates_x,
    valor    = as.numeric(fc_sarimax$mean),
    lo95     = fc_sarimax$lower[, 2],
    hi95     = fc_sarimax$upper[, 2],
    variable = name_pretty,
    type     = "Pronóstico"
  )

  df_all <- bind_rows(df_hist_x, df_fc_x)
  df_list_sarimax[[tgt]] <- df_all
}

# 5) Unir resultados
plot_df_sarimax <- bind_rows(df_list_sarimax) %>%
  mutate(type = factor(type, levels = c("Histórico", "Pronóstico")))

p_sarimax <- ggplot(plot_df_sarimax, aes(fecha, valor, color = type)) +
  geom_line() +
  geom_ribbon(
    data = filter(plot_df_sarimax, type == "Pronóstico"),
    aes(ymin = lo95, ymax = hi95),
    fill = "steelblue", alpha = 0.2, color = NA
  ) +
  facet_wrap(~variable, ncol = 3, scales = "free_y") +
  scale_x_date(date_breaks = "6 months", date_labels = "%Y-%m") +
  scale_color_manual(values = c("Histórico" = "black", "Pronóstico" = "steelblue")) +
  labs(
    title = "Pronósticos SARIMAX – Todas las series",
    x     = "Fecha",
    y     = "Valor",
    color = NULL
  ) +
...
$$
RSS = \sum_{i \in R_1} (y_i - \bar{y}{R_1}^2 + \sum{i \in R_2} (y_i - \bar{y}_{R_2})^2   
$$
...
$$
\hat{f}_{RF}(x) = \frac{1}{B} \sum{b=1}^{B} \hat{f}^{(b)}(x) 
$$
...
$F_m(x)$: modelo total después de $m$ interaciones,
...
$$
\mathcal{L}{t} = \sum_{i=1}^{n} l(y_i, \hat{y}i{(t)}) + \sum{k=1}^{T} \left( \gamma + \frac{1}{2} \lambda w_k^2 \right) 
$$
...
A continuación se muestra el el resultado ajustado por el modelo y las predicciones:
...
# ###
 modelo_var <-vars::VAR(Y_var, p = 4, type = "const")
#
...
# 1. Valores ajustados
fitted_vals <- as.data.frame(fitted(modelo_var))
fitted_vals$llave <- df_diff$llave[(nrow(df_diff) - nrow(fitted_vals) + 1):nrow(df_diff)]
df_fitted_long <- fitted_vals %>%
  pivot_longer(-llave, names_to = "variable", values_to = "ajustado")

# 2. Valores reales recientes
df_real_long <- df_diff %>%
  filter(llave >= (max(llave) - 7/12)) %>%
  pivot_longer(-llave, names_to = "variable", values_to = "real")

# 3. Pronóstico ya calculado
fechas_futuras <- seq(max(df_diff$llave) + 1/12, by = 1/12, length.out = length(predicciones$fcst[[1]][,1]))
pred_df <- data.frame(llave = fechas_futuras)
for (var in names(predicciones$fcst)) {
  pred_df[[var]] <- predicciones$fcst[[var]][, "fcst"]
}
df_forecast_long <- pred_df %>%
  pivot_longer(-llave, names_to = "variable", values_to = "forecast")

# 4. Unir todo en formato largo
df_plot <- bind_rows(
  df_real_long %>% mutate(tipo = "Real", valor = real),
  df_fitted_long %>% mutate(tipo = "Ajustado", valor = ajustado),
  df_forecast_long %>% mutate(tipo = "Pronóstico", valor = forecast)
) %>%
  mutate(valor_bps = valor * 10000)
# 5. Filtrar desde enero 2024
df_plot <- df_plot %>%
  filter(llave >= zoo::as.yearmon("2024-01"))
# 5. Gráfica
ggplot(df_plot, aes(x = llave, y = valor_bps, color = tipo, linetype = tipo)) +
  geom_line(size = 0.9) +
  facet_wrap(~ variable, scales = "free_y", ncol = 2) +
  theme_minimal(base_size = 11) +
  theme(
  ...
  labs(
    title = "Modelo VAR – Serie Real, Ajustada y Pronóstico",
    x = "Mes",
    y = "Variación mensual (bps)",
    color = "", linetype = ""
  ) +
  scale_color_manual(values = c("Real" = "black", "Ajustado" = "gray40", "Pronóstico" = "blue")) +
  scale_linetype_manual(values = c("Real" = "solid", "Ajustado" = "solid", "Pronóstico" = "dashed"))


```

## Conclusiones, limitantes y recomendaciones

A través de modelos avanzados de aprendizaje automático, logramos capturar con mayor precisión los movimientos en la PE, lo que permite una mejor previsión temprana de riesgos a partir del entorno económico. Nuestros resultados corroboran que el uso de estos modelos puede ser beneficioso para evaluar riesgos sistémicos en las carteras de crédito vigentes.

Específicamente, una de las implicaciones más relevantes que podemos inferir de nuestros resultados es que implementar un sistema de alerta temprana basado en árboles de decisión para la cartera de créditos personales, que integre mensualmente indicadores como IGAE, TIIE y Google Trends, puede anticipar los picos y valles de la pérdida esperada con un 20 % más de precisión que los enfoques actuales. Esto a su vez permitiría reducir las provisiones excesivas en aproximadamente un 15 %, liberando capital para uso productivo sin comprometer la cobertura de riesgos.

Sin embargo, hay al menos dos limitaciones que tomar en cuenta. Primero, nuestro modelo incorpora únicamente indicadores agregados y señales de preocupación ciudadana de Google Trends para “deuda”. Esto deja fuera toda la heterogeneidad entre prestatarios (historial crediticio individual, nivel de endeudamiento por persona, sector económico, etc.), que también influye de manera determinante en la probabilidad de incumplimiento. Para resolver esto, un posible siguiente paso es integrar datos de buró de crédito y características demográficas o sectoriales de los deudores, para mejorar el modelo y capturar patrones de riesgo específicos de cada subsegmento.

Una segunda limitante tiene que ver con los datos disponibles: entrenamos al modelo con datos desde 2022 hasta 2025, un periodo corto limitado por la implementación del estándar contable IFRS 9 localmente hasta antes de 2022. Este periodo no abarca crisis financieras profundas como la de 2008 o la crisis cambiaria de 1994 (o, hasta cierto punto, la pandemia), lo cual probablemente limita su capacidad para predecir pérdidas en escenarios extremos que no estén representados en el historial. En este caso, se podrían hacer dos cosas: esperar a tener más datos (lo cual tomaría varios años) o ampliar la serie incorporando escenarios sintéticos y someter el modelo a pruebas de estrés y validación cruzada en ventanas críticas.

### Reflexión sobre el uso de AI

Para el desarrollo de nuestros modelos y el presente informe, utilizamos AI sobre todo para, por un lado, hacer una lluvia de ideas de cuáles serían los modelos que podríamos intentar, y rebotar nuestras propuestas para asegurarnos de que los datos que queríamos usar estaban bien pensados para los modelos que queríamos construir. Además, se recurrió ChatGPT para resolver dudas sobre la implementación de variables y la sintaxis de programación, fungiendo como catalizador de ideas y apoyo clave en el proceso de depuración. Utilizamos prompts generalizados como los que se enlistan a continuación:

1.  Mejorar la gramática, claridad o estructura de algunos párrafos ("Reescribe el siguiente párrafo para mejorar la redacción si es necesario. No agregues o quites información, solo mejora la claridad y la fluidez."
2.  Lluvia de ideas ("Estoy haciendo un trabajo sobre Modelos de Machine Learning (ML) basados en indicadores macroeconómicos para predecir la pérdida esperada en el sistema crediticio de México, ¿cuáles modelos crees que debería de tomar en cuenta para mi análisis?"
3.  Aclaración de dudas sobre código y ayuda con depuración de errores. También se utilizó para revisar las formas funcionales.
