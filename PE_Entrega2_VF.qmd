...
pacman::p_load(tidyverse, glue, readxl, janitor, lubridate, scales, data.table, kableExtra, tseries, gt, gtExtras, zoo, tibble, knitr, gridExtra, corrplot, caret, rpart, rpart.plot, randomForest, xgboost, rlang, prophet, Metrics, forecast,ggfortify, Metrics)
```
...
5.  **VAR**
    ...
    El modelo VAR (Vector Autoregresivo) es una herramienta econométrica diseñada para capturar las interrelaciones dinámicas entre múltiples series temporales. A diferencia de los modelos univariados, el VAR permite que cada variable del sistema sea explicada no solo por sus propios rezagos, sino también por los rezagos de las demás variables incluidas. El supuesto fundamental es que todas las variables son tratadas como endógenas y que la estructura de rezagos resume adecuadamente la dinámica del sistema.
...
```{r}
# — 0) Cargar librerías y limpiar workspace
rm(list = ls())
if (!require("pacman")) install.packages("pacman")
pacman::p_load(dplyr,ggplot2, stringr, tidyr)


# — 2) Leer y limpiar datos
I <- read_excel("master_base_filtrado.xlsx") %>%
  clean_names() %>%
  mutate(
    m1               = m1 / 1e6,
    m2               = m2 / 1e6,
...
# — 3) Definir series objetivo y exógenas
targets <- c(
...
# — 1) Listas vacías para apilar resultados
#df_list_sarima  <- list()
df_list_sarimax <- list()

# — 2) Loop sobre cada serie objetivo
for (tgt in targets) {
  # Nombre legible para títulos y facetas
  name_pretty <- str_to_title(str_replace_all(tgt, "y_pe_|_", " "))
  
  # — Datos históricos sin NAs
  df0 <- I %>% dplyr::select(llave, all_of(tgt)) %>% drop_na()
  ts0 <- ts(df0[[tgt]],
            start     = c(year(min(df0$llave)), month(min(df0$llave))),
            frequency = 12)
  
  # —— A) SARIMA puro (seasonal=TRUE) ——
 # fit_sarima  <- auto.arima(ts0, seasonal = TRUE)
 # fc_sarima   <- forecast(fit_sarima, h = 12)
  
  # Gráfica individual SARIMA
 # plot(fc_sarima,
 #      main = paste0("SARIMA – ", name_pretty),
 #      xlab = "Año", ylab = "Valor")
  #lines(ts0, col = "black")
  
  # Preparo tibble histórico + forecast
# df_hist_s   <- tibble(
#   fecha    = df0$llave,
#   valor    = df0[[tgt]],
...
#   type     = "Histórico"
# )
# fdates_s    <- seq(max(df0$llave) %m+% months(1),
#                    by = "month", length.out = 12)
# df_fc_s     <- tibble(
#   fecha    = fdates_s,
#   valor    = as.numeric(fc_sarima$mean),
#   lo95     = fc_sarima$lower[,2],
#   hi95     = fc_sarima$upper[,2],
#   variable = name_pretty,
#   type     = "Pronóstico"
# )
# df_list_sarima[[tgt]] <- bind_rows(df_hist_s, df_fc_s)
# 
  # —— B) SARIMAX con exógenas ——
  df1 <- I %>% dplyr::select(llave, all_of(tgt), all_of(exogs)) %>% drop_na()
  # Aseguro numéricas
  df1[exogs] <- lapply(df1[exogs], as.numeric)
  ts1 <- ts(df1[[tgt]],
            start     = c(year(min(df1$llave)), month(min(df1$llave))),
            frequency = 12)
  xreg1 <- as.matrix(df1[exogs])
  
  fit_sarimax <- auto.arima(ts1, xreg = xreg1, seasonal = TRUE)
  fc_sarimax  <- forecast(fit_sarimax, xreg = tail(xreg1,12), h = 12)
  
  # Gráfica individual SARIMAX
  plot(fc_sarimax,
       main = paste0("SARIMAX – ", name_pretty),
       xlab = "Año", ylab = "Valor")
  lines(ts1, col = "black")
  ...
  fdates_x    <- seq(max(df1$llave) %m+% months(1),
                     by = "month", length.out = 12)
  df_fc_x     <- tibble(
    fecha    = fdates_x,
  ...
  df_list_sarimax[[tgt]] <- bind_rows(df_hist_x, df_fc_x)
}

# — 3) Crear data.frames para facetas
#plot_df_sarima  <- bind_rows(df_list_sarima)  %>%
#  mutate(type = factor(type, levels = c("Histórico"#,"Pronóstico")))
plot_df_sarimax <- bind_rows(df_list_sarimax) %>%
  mutate(type = factor(type, levels = c("Histórico","Pronóstico")))

## — 4) Facetas SARIMA (2 filas × 3 columnas)
#p_sarima <- ggplot(plot_df_sarima, aes(fecha, valor, #color = type)) +
#  geom_line() +
#  geom_ribbon(
#    data = filter(plot_df_sarima, type == "Pronóstico"),
...
#  labs(
#    title = "Pronósticos SARIMA – Todas las series",
#    x     = "Fecha",
#    y     = "Valor",
#    color = NULL
#  ) +
...
#print(p_sarima)
#
# — 5) Facetas SARIMAX (2 filas × 3 columnas)
